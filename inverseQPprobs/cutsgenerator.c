					   ...
					   int *lpcc_stat)
{
	...
	*lpcc_stat=LPCC_PRECONDITION_BOUNDED;
	...
	case LPCC_PRECONDITION_OPT:
		fprintf(stdout,"	After adding %i disjunctive cuts, find optimal solution %f \n",cut_cnt,lp_obj);
		break;
	case LPCC_PRECONDITION_BOUNDED:
		...
		if(status)goto TERMINATE;
		fprintf(stdout,"	After adding %i disjunctive cuts, improved lp relaxation is %f\n",cut_cnt,lp_obj);
		break;
	case LPCC_PRECONDITION_INFEASIBLE:
		fprintf(stdout,"	After adding %i disjunctive cuts, find LPCC is infeasible \n",cut_cnt);
		break;
	default:
		status=-1;
				  ...
				  int *lpcc_stat)
{
	...
	*lpcc_stat=LPCC_PRECONDITION_BOUNDED;
	...
	{
	case LPCC_PRECONDITION_OPT:
		fprintf(stdout,"	After adding %i simple cuts, find optimal solution %f \n",cut_cnt,lp_obj);
		break;
	case LPCC_PRECONDITION_BOUNDED:
		status=copy_startinfo(&updated_rx_lp_start,lp_start);
		if(status)goto TERMINATE;
		fprintf(stdout,"	After adding %i simple cuts, improved lp relaxation is %f\n",cut_cnt,lp_obj);
		break;
	case LPCC_PRECONDITION_INFEASIBLE:
		fprintf(stdout,"	After adding %i simple cuts, find LPCC is infeasible \n",cut_cnt);
		break;
	default:
		status=-1;
						...
						CONSTRAINT_SET *cut_ptr)
{
	...
	for (i=0;i<param_m;i++)
	{
		...
		//fprintf(stdout,"i: %i, cur_w_ub: %f, cur_y_ub: %f\n",i,w_ub[i],y_ub[i]);
	...
	*lpcc_stat=LPCC_PRECONDITION_BOUNDED;
	...
	for (i=0;i<loop_limit;i++)
	{
		...
		case CPX_STAT_OPTIMAL:
			...
			{
				//add bound cut
				status=boundcuts_generator(env,rx_lp,rx_lp_start,param_n,param_m,param_k,lpcc_ub,num_var,select_index,param_p,&cut_cnt,&cut_set);
				if(status) goto TERMINATE;
	...
	{
	case LPCC_PRECONDITION_OPT:
		fprintf(stdout,"	After adding %i bound cuts, find optimal solution %f \n",cut_cnt, lp_obj);
		break;
	case LPCC_PRECONDITION_BOUNDED:
		status=copy_startinfo(&updated_rx_lp_start,lp_start);
		if(status)goto TERMINATE;
		fprintf(stdout,"	After adding %i bound cuts, improved lp relaxation is %f\n",cut_cnt, lp_obj);
		break;
	case LPCC_PRECONDITION_INFEASIBLE:
		fprintf(stdout,"	After adding %i bound cuts, find lpcc is infeasible \n", cut_cnt);
		break;
	default:
		status=-1;
